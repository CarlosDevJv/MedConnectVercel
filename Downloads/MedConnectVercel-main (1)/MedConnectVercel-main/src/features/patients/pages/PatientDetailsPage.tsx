...
export function PatientDetailsPage() {
  ...
  // Hook ANTES de early returns
  const patientForContext = !query.isLoading && !query.isError ? query.data : null
  useLuziaPageContext(
    patientForContext
      ? {
          tela: 'Ficha de Paciente',
          paciente_id: patientForContext.id,
          nome_paciente: patientForContext.full_name,
          cpf: patientForContext.cpf,
          alergias: patientForContext.allergies || 'Nenhuma alergia cadastrada',
          convenio: patientForContext.insurance_company || 'Particular',
          tipo_sanguineo: patientForContext.blood_type || 'Não informado',
          observacoes: patientForContext.notes || 'Nenhuma observação',
        }
      : {}
  )

  // TODOS os useMemo ANTES de early returns
  const photoBase64 = React.useMemo(() => {
    try {
      return patientForContext ? localStorage.getItem(`mediconnect.patient.photo.${patientForContext.id}`) : null
    } catch {
      return null
    }
  }, [patientForContext?.id])
